...
export default function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/30 bg-white/16 backdrop-blur-xl">
                  ...
                  <>
                    <div className="max-h-64 overflow-y-auto">
                      {cart.slice(0, 3).map((item, index) => (
                        <div key={item.cartKey || item.slug || index} className="flex items-center gap-3 p-4 border-b border-gray-50 last:border-b-0">
                          ...
                          <button
                            ...
                            className="text-gray-400 hover:text-red-500 transition-colors flex-shrink-0 text-xs font-semibold"
                          >
                            Remove
                          </button>
                        </div>
                      ...
                      {cart.length > 3 && (
                        <p className="text-center text-sm text-gray-500 py-2">+{cart.length - 3} more item{cart.length - 3 > 1 ? 's' : ''}</p>
                      )}
                    </div>
      ...
      {menuOpen && (
        <nav className="md:hidden bg-white border-t border-brand-lavender px-6 py-4">
